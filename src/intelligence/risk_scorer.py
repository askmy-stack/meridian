...
class FeatureVector:
    ...
    # Geographic features
    conflict_proximity_score: float = 0.0  # 0-1, distance to nearest conflict
    political_stability_index: float = 0.5  # 0-1, higher = more stable
    
    # Operational features
    port_congestion_score: float = 0.0  # 0-1, higher = more congested
    weather_risk_score: float = 0.0  # 0-1, extreme weather probability
    
    # Event features
    recent_events_count: int = 0  # Events in last 30 days
    critical_events_count: int = 0  # CRITICAL/HIGH events in last 30 days
    
    # Network features
    dependency_depth: int = 1  # Supply chain tier
    single_source_flag: bool = False
    alternative_sources_count: int = 0
    
    # Financial features
    supplier_financial_health: float = 0.5  # 0-1, higher = healthier
    market_volatility_index: float = 0.0  # 0-1
    
    # Historical features
    historical_disruption_count: int = 0  # Past 12 months
    ...
    def to_array(self) -> np.ndarray:
        ...
        return np.array([
            ...
            min(self.recent_events_count / 10, 1.0),  # Normalize
            min(self.critical_events_count / 5, 1.0),  # Normalize
            min(self.dependency_depth / 5, 1.0),  # Normalize
            float(self.single_source_flag),
            min(self.alternative_sources_count / 5, 1.0),  # Normalize
            self.supplier_financial_health,
            self.market_volatility_index,
            min(self.historical_disruption_count / 5, 1.0),  # Normalize
            min(self.avg_resolution_time_days / 30, 1.0),  # Normalize
        ])
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        return {
            ...
            "historical_disruption_count_normalized": min(self.historical_disruption_count / 5, 1.0),
